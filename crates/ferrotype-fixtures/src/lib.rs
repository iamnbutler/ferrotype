...
mod derive_tests {
    ...
    #[test]
    fn test_auto_registration_collects_types() {
        use ferrotype::TypeRegistry;

        // TypeRegistry::collect() gathers all auto-registered types from the distributed slice
        let registry = TypeRegistry::collect();

        // Non-generic derived types should be auto-registered
        assert!(registry.get("DerivedStatus").is_some(), "DerivedStatus should be auto-registered");
        assert!(registry.get("DerivedCoordinate").is_some(), "DerivedCoordinate should be auto-registered");
        assert!(registry.get("DerivedShape").is_some(), "DerivedShape should be auto-registered");
        assert!(registry.get("DerivedMessage").is_some(), "DerivedMessage should be auto-registered");
        assert!(registry.get("DerivedRpcError").is_some(), "DerivedRpcError should be auto-registered");

        // Generic types like DerivedOptionalValue<T> are NOT auto-registered
        // because we don't know which concrete instantiations will be used
        assert!(registry.get("DerivedOptionalValue").is_none(), "Generic types should not be auto-registered");

        // Verify the registry has at least the expected types
        assert!(registry.len() >= 5, "Should have at least 5 auto-registered types, found {}", registry.len());
    }

    #[test]
    fn test_auto_registration_renders_correctly() {
        use ferrotype::TypeRegistry;

        let registry = TypeRegistry::collect();
        let output = registry.render();

        // Output should contain the auto-registered types
        assert!(output.contains("DerivedStatus"), "Output should contain DerivedStatus");
        assert!(output.contains("DerivedCoordinate"), "Output should contain DerivedCoordinate");

        // Output should have the header comment
        assert!(output.contains("// Generated by ferrotype"), "Output should have header comment");
    }
}
